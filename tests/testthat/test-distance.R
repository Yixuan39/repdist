...
test_that("sample_repdist takes the embedding matrix, not a precomputed dist", {
    expect_error(
        sample_repdist(counts, stats::dist(Z)),
        "embedding matrix"
    )
    expect_error(
        sample_repdist(counts, repdist_matrix(Z / sqrt(rowSums(Z^2)))),
        "embedding matrix"
    )
})
...
test_that("a degenerate bandwidth is an error, not a NaN kernel", {
    ...
    expect_true(all(is.finite(sample_repdist(counts, near))))
    expect_error(sample_repdist(counts, Z, sigma = 0), "positive")
    expect_error(sample_repdist(counts, Z, sigma = -1), "positive")
})

test_that("Euclidean MMD survives translation and close pairs", {
    z <- rbind(p1 = c(0, 0), p2 = c(1, 0))
    cc <- diag(2)
    dimnames(cc) <- list(c("s1", "s2"), rownames(z))
    expected <- sqrt(2 * (1 - exp(-0.5)))
    for (offset in c(0, 1e8)) {
        expect_equal(
            as.numeric(sample_repdist(cc, z + offset, sigma = 1)),
            expected
        )
        expect_equal(as.numeric(sample_repdist(cc, z + offset)), expected)
    }
    # A far-away third protein prevents centering alone from fixing close pairs.
    z <- rbind(p1 = c(0, 0), p2 = c(1e-9, 0), p3 = c(1, 0))
    cc <- cbind(cc, p3 = 1)
    expect_equal(as.numeric(sample_repdist(cc, z, sigma = 1e-9)),
        expected / 2,
        tolerance = 1e-7
    )
    for (bad in list("1", TRUE, NaN, Inf, NULL)) {
        if (!is.null(bad)) {
            expect_error(sample_repdist(cc, z, sigma = bad), "sigma")
        }
    }
})
...
test_that("sample and protein IDs are valid before alignment", {
    for (bad in list(c("x", "x"), c(NA, "y"), c(" ", "y"))) {
        ...
        rownames(cc) <- bad
        expect_error(sample_repdist(cc, Z), "names")
        cc <- counts[, 1:2, drop = FALSE]
        colnames(cc) <- bad
        expect_error(sample_repdist(cc, Z), "names")
    }
...
test_that("mmd_matrix rejects materially negative squared distances", {
    ...
    expect_error(
        repdist:::mmd_matrix(tcrossprod(P %*% K, P)),
...
test_that("repdist_matrix computes inner-product distances on unit-norm rows", {
    ...
    expect_equal(
        as.vector(repdist_matrix(rbind(
            c(1, 0), c(1, 0),
            c(0, 1), c(-1, 0)
        ))),
        c(0, 1, 2, 1, 2, 1)
