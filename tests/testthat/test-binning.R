...
test_that("bin_proteins returns clusters, similarity, representatives, tree", {
    ...
    expect_named(out, c(
        "clusters", "similarity", "representatives", "tree",
        "min_sim"
    ))
...
test_that("the partition is exactly a complete-linkage cut of the distance", {
    ...
    membership <- setNames(
        rep(names(out$clusters), lengths(out$clusters)),
        unlist(out$clusters)
    )[rownames(Z)]
...
test_that("a square distance matrix is accepted", {
    expect_equal(
        bin_proteins(as.matrix(D), min_sim = 0.5)$clusters,
