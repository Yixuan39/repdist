utils::globalVariables(c(
    "similarity", "min_sim", "value", "curve", "metric",
    "protein", "partner"
))
...
.repdist_check_ids <- function(ids, what) {
...
# repdist_matrix() records method = "cosine"; stats::dist() records
# "euclidean". A euclidean `dist` here means the caller passed embeddings
# straight to dist(), whose values are not on the 1 - similarity scale.
.repdist_similarity_dist <- function(D) {
...
# Accessions are long, and the similarity matrix is symmetric, so one axis
# carries them, elided in the middle: both ends are informative in an
# assembly-derived name.
.repdist_elide <- function(x, name_max) {
    ifelse(nchar(x) <= name_max, x, paste0(
...
#' ground metric -- for TM-Vec embeddings, a predicted TM-score. A pile-up at
#' 1 means near-duplicate proteins, or an embedding model too saturated to
#' resolve similarity thresholds; every protein left of a candidate
#' `min_sim` becomes a singleton bin in
#' [bin_proteins()].
#'
#' @param D Protein `dist` from
#'   [repdist_matrix()], or a square matrix on
#'   the same `1 - similarity` scale. A euclidean `dist` is
#'   rejected.
...
plot_similarity_profile <- function(D, min_sim = NULL) {
    if (!requireNamespace("ggplot2", quietly = TRUE)) {
        stop("plot_similarity_profile() needs the ggplot2 package.",
            call. = FALSE)
    }
    D <- .repdist_similarity_dist(D)
    stopifnot(attr(D, "Size") >= 2L)

    # ponytail: expands the condensed dist to a square matrix; switch to a
    # blocked scan (see .repdist_medoid) if QC at the 46,341-protein ceiling
    # ever matters.
    M <- as.matrix(D)
    diag(M) <- NA
...
#'
#' Sweeps `min_sim` over the complete-linkage tree
#' [bin_proteins()] already computed -- no
#' reclustering. The lower panel brackets the two ways binning stops being
#' informative: proteins collapsing into one bin as the threshold drops is
#' chaining, and proteins falling out as singletons as it rises is no
#' aggregation at all. A usable threshold sits in the valley between the two
#' curves, on a plateau of the bin count.
#'
#' @param bins Result of [bin_proteins()], or its
#'   `tree` ([stats::hclust]).
...
plot_bin_profile <- function(bins, min_sim = NULL) {
    ...
    n <- nrow(cut)
    share <- function(f) {
        100 * apply(cut, 2, function(b) sum(f(tabulate(b)))) / n
    }
    df <- data.frame(
        min_sim = rep(grid, 3L),
        value = c(apply(cut, 2, max), share(max), share(function(k) k == 1L)),
        curve = rep(c("bins", "largest bin", "singletons"),
            each = length(grid)),
...
# The members of one named bin, ordered as the tree draws them so the heatmap
# reads down the diagonal.
.repdist_bin_members <- function(bins, bin) {
    ...
    if (!is.character(bin) || length(bin) != 1L ||
        ...
        stop("`bin` must name one of the ", length(bins$clusters),
            " bins in `bins$clusters`, e.g. \"bin1\".",
            call. = FALSE
        )
    }
    ...
    members[order(match(members, bins$tree$labels[bins$tree$order]))]
}
...
#' [bin_proteins()] guarantees the whole block sits
#' at or above `min_sim`; this is where you see how far above, and
#' whether a bin is one tight group or two sub-groups that only just met the
#' threshold. The fill scale starts at that threshold, so bins are comparable
#' across calls.
#'
#' @details The whole square is drawn, with the diagonal running from the
#' upper left to the lower right. The matrix is symmetric, so each pair
#' appears twice; that redundancy is what lets a row be read straight across,
#' and keeps a sub-group that straddles the diagonal square rather than
#' triangular.
#'
...
plot_bin_similarity <- function(bins, bin, D, label_max = 20L,
    ...
    members <- .repdist_bin_members(bins, bin)
    D <- .repdist_similarity_dist(D)
    n <- length(members)
    ...
    p <- ggplot2::ggplot(df,
        ...
        ggplot2::scale_y_discrete(
            labels = function(x) .repdist_elide(x, name_max)
        ) +
        ggplot2::coord_fixed() +
