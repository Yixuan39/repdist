...
# The validated `dist` bin_proteins() clusters: a square matrix is coerced,
# and the values must be cosine distances in [0, 2] over named proteins.
.repdist_ground_dist <- function(D) {
    if (!inherits(D, "dist")) {
        if (!is.matrix(D) || !is.numeric(D) || nrow(D) != ncol(D) ||
            !isSymmetric(D) || any(diag(D) != 0)) {
            stop("`D` must be a symmetric numeric distance matrix ",
                "with zero diagonal.",
                call. = FALSE
            )
        }
    ...
    n <- attr(D, "Size")
    stopifnot(
        length(n) == 1L, is.finite(n), n >= 2L, n == floor(n),
    ...
    # sum(), not all(is.finite()): the latter allocates a logical the size of
    # the ground metric. Distances are non-negative, so any NA/NaN/Inf
    # reaches the sum.
    if (!is.finite(sum(D))) {
...
#'
#' Cuts a complete-linkage tree over a
#' [repdist_matrix()] cosine distance at
#' `1 - min_sim`, so every within-bin pair has similarity at least
#' `min_sim`. Bins are named `bin1`, `bin2`, ... from largest
#' to smallest.
#'
#' Structural similarity is not shared function: weigh annotation agreement
#' against how many proteins the non-singleton bins hold, and how far the
#' families fragment, before transferring annotations to uncharacterised
#' members.
#'
#' Collapse a sample-by-protein counts table to bin level with the returned
#' membership, e.g.
#' `t(rowsum(t(counts), bin[colnames(counts)]))`,
#' where `bin` maps a protein to its bin name.
#'
#' @param D Protein `dist` from
#'   [repdist_matrix()], or a square distance
#'   matrix on the same `1 - similarity` scale. A euclidean `dist`
#'   is rejected, because `1 - min_sim` would be a meaningless cut height
#'   on it.
#' @param min_sim All-pairs similarity threshold in `(0, 1]` (default
#'   0.7).
#' @return A list:
#'   \describe{
...
#'     \item{`tree`}{The complete-linkage [stats::hclust] tree
#'       over all proteins. Cut it at `1 - s` with
#'       [stats::cutree()] to explore other thresholds without
#'       reclustering.}
...
bin_proteins <- function(D, min_sim = 0.7) {
    ...
    D <- .repdist_ground_dist(D)

    tree <- fastcluster::hclust(D, method = "complete")
    clusters <- unname(split(
        attr(D, "Labels"), stats::cutree(tree, h = 1 - min_sim)
    ))
